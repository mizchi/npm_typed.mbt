...
The main entry point is the `tagged` function, which works like the `$` template literal in JavaScript zx.

```mbt nocheck
// Execute shell commands with .run()
let output = @zx.tagged("echo hello").run()
println(output.text()) // "hello"

// With variable interpolation using ${@0}, ${@1}, etc.
// Use @zx.arg() to convert values to zx arguments
let name = "world"
let output = @zx.tagged("echo ${@0}", args=[@zx.arg(name)]).run()
println(output.text()) // "world"

// Multiple arguments with different types
let count = 42
let output = @zx.tagged("echo ${@0} ${@1}", args=[@zx.arg(name), @zx.arg(count)]).run()
```

## ProcessPromise Methods

The `tagged` function returns a `ProcessPromise` that can be:

```mbt nocheck
// Simple execution with .run()

///|
let output = @zx.tagged("ls -la").run()

// Chained with modifiers

///|
let output = @zx.tagged("some-command")
...
///|
let text = @zx.tagged("echo hello").text().wait()

///|
let lines = @zx.tagged("ls").lines().wait()

///|
let exit_code = @zx.tagged("exit 0").exit_code().wait()
```

## ProcessOutput Properties

```mbt nocheck
let output = @zx.tagged("echo hello").to_promise().wait()
...
let result = @zx.retry(3, fn() {
  @zx.tagged("some-flaky-command").to_promise()
}).wait()
```
...
```mbt nocheck
///|
let output = @zx.tagged("echo 'hello world'")
  .pipe(@zx.tagged("grep hello"))
  .run()
```
...
let output = @zx.tagged("exit 1").run() // throws!

// This won't throw
let output = @zx.tagged("exit 1").nothrow().run()
...
///|
let output = @zx.tagged("head -n ${@0} ${@1}", args=[
  @zx.arg(lines),
  @zx.arg(name),
]).run()
...
|----------|-------------|
| `tagged(cmd, args?)` | Execute shell command |
| `execa(cmd, args?)` | Execute and wait (async convenience) |
